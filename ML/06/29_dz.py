...
# Влияние параметра max_depth:  
    ...
    # Дерево решений — это непараметрический метод машинного обучения, который рекурсивно разбивает данные на подгруппы на основе выбранного критерия (например, Gini или Entropy). На каждом этапе алгоритм стремится найти такой признак и такое значение порога, которые лучше всего разделяют классы. 
    ...
    # Таким образом, дерево решений позволяет легко интерпретировать модель и понимать, какие признаки и как влияют на финальное решение. 
